#setwd("G:/A-Dr.Seq-Project")
a <-commandArgs(T)
#a <- c("final_test_ATAC_signal.txt","final_test_ATAC","0","10","10","final_test_ATAC.readsnumber_cell.txt",2)
input_file <- a[1]
outname <- a[2]
...
cell_labels <- reads_number_per_cell[,2]
...
if (length(which(is.na(silScore)))){
...
allPlotMatrix[allPlotMatrix<zmin] <- zmin
allPlotMatrix[allPlotMatrix>zmax] <- zmax
