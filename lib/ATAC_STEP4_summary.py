...
\\begin{document}
\\title{Dr.seq 2.2 QC and Analysis Summary Report: %s}
...
"""%(LatexFormat(conf_dict['General']['outname']))
    ...
    if int(conf_dict['Step1_Mapping']['q30filter']) == 1:
        ...
        q30filter = "False"
    if int(conf_dict["Step1_Mapping"]["filter_reads_length"]) == 1:
        filter_reads_length = "True"
    else:
        filter_reads_length = "False"     
    QCdoc += """      
\hline
parameter & value  \\\\
\hline
output name & %s \\\\
\hline
reads file 1(file name only) & %s \\\\
\hline
reads file 2(file name only) & %s \\\\
\hline
...
In the bulk-cell level QC step we measured the performance of total ATAC reads. In this step we did't separate reads, just like treated the sample as a bulk ATAC-seq sample.
\\newpage
\\newpage
\subsection{Reads alignment summary}
\\begin{quotation}
The following table shows read pairs number after each filter strategy and mapped reads of final selected reads. It measures the general sequencing quality. Low mappability indicates poor sequence quality(see ``Reads level QC") or library quality(caused by contaminant). In summary, if the percentage of ``total mapped reads" is less than 5\\%%, users may consider reconstruct your library(redo the experiment), but first you should make sure you already trim the adapter and map your reads to the corresponded species(genome version). Mappable reads was after Q30 filtering if Q30 filter function was turned on. The insert size distribution of sequenced fragments from chromatin had clear periodicity of approximately 200 bp, because many fragments are protected by integer multiples of nucleosomes. Reads containing integer multiples of nucleosomes was removed by filtering  reads length shorter than 100bp ,if reads length filter function was turned on. \\\\
\end{quotation}
\\begin{table}[h]
...
\hline
genomic region(Category) &  read pairs number \\\\
\hline
total number of read pairs & %s \\\\
\hline
number of reads pairs after barcode filtered &  %s (%s\\%%)* \\\\
\hline
number of mapped read paird & %s (%s\\%%)* \\\\
\hline
number of read pairs after length filtered & %s (%s\\%%)* \\\\
\hline
number of mitochondria read pairs & %s (%s\\%%)* \\\\
\hline
...
"""%(NumberFormat(str(conf_dict['Step2_QC']['total_reads_pair_N'])),
     ...
     NumberFormat(str(conf_dict['Step2_QC']['m_reads_N'])),
     str(round(100*int(conf_dict['Step2_QC']['m_reads_N'])*1.0/int(conf_dict['Step2_QC']['total_reads_pair_N']), 2)))

### reads on chromsome
    QCdoc += """
\\newpage
\\newpage
\subsection{Chromosomal Distribution of ATAC Regions}
\\begin{quotation}
The blue bars represent the percentages of the whole tiled or mappable regions in the chromosomes (genome background) and the red bars showed the percentages of the whole ATAC. These percentages are also marked right next to the bars. P-values for the significance of the relative enrichment of ATAC regions with respect to the gnome background are shown in parentheses next to the percentages of the red bars.
\end{quotation}
\\begin{figure}[h]
        \caption{Chromosomal Distribution of ATAC Regions} \label{fig:profileunion}
        \setlength{\\abovecaptionskip}{0pt}
        \setlength{\\belowcaptionskip}{10pt}
        \centering
        {\includegraphics[width=0.8\\textwidth]{%s}}
...
\\begin{quotation}
Barplot show ATAC regions distributed over the genome along with their scores or peak heights. The line graph on the top left corner illustrates the distribution of peak heights (or scores). The x-axis of the main plot represents the actual chromosome sizes.
\end{quotation}
\\begin{figure}[h]
...
\\begin{quotation}
Average profiling within/near important genomic features. The panels on the first row display the average ATAC enrichment signals around TSS and TTS of genes, respectively. The bottom panel represents the average ATAC signals on the meta-gene of 5 kb.
\end{quotation}
\\begin{figure}[h]
